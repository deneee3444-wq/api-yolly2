...
def generate_ai_video_service(
    ...
    for i in range(max_attempts):
        ...
        if status == "Done":
            # Attempt to fetch the actual video files and decryption keys from info/consume/.../tasks/files
            try:
                ...
                if consume_files:
                    ...
                    for idx, f in enumerate(consume_files):
                        furl = f.get("url", "")
                        task_info = f.get("task", {})
                        enc_key = task_info.get("permanent_key")
                        enc_iv = task_info.get("permanent_iv")
